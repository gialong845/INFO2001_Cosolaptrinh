...
void arrangeNumber() {
    ...
    for (int i=0; i<quantity-1; i++) {
        for (int j=i+1; j<quantity; j++) {
            if (listStudents[i].id > listStudents[j].id) {
                temp = listStudents[i];
                listStudents[i] = listStudents[j];
                listStudents[j] = temp;
            }
...
int main(){
    ...
    do {
        ...
        switch (List){
            ...
            case 3:
              arrangeName();
              printf("Hoan thanh sap xep danh sach theo ten\n");
              break;
            case 4:
              arrangeScore();
              printf("Hoan thanh sap xep danh sach theo diem\n");
              break;
            case 5:
              arrangeNumber();
              printf("Hoan thanh sap xep danh sach theo ma so\n");
              break;
            case 0:
